    ...
    <div className='w-full flex'>
      ...
      <div className="sec w-full sm:w-[80%] bg-[#f6f7f9] p-4 sm:p-6  flex flex-col gap-10 font-[family-name:var(--font-geist-sans)]">
        <section className="w-full flex flex-col sm:flex-row items-center justify-center sm:justify-between ">
          <Image src={"/Images/pickup.svg"} alt="" width={582} height={132} className="w-[200px] md:w-[270px] lg:w-[582px]" />
          <Image src={"/Images/switch.svg"} alt="" width={60} height={60} className="w-[80px]" />
          <Image src={"/Images/pickup.svg"} alt="" width={582} height={132} className=' w-[200px] md:w-[270px] lg:w-[582px]' />
        </section>
        <section className="popular w-full flex flex-col gap-4">
          <div className="sec grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
            {[
              { title: 'Koenigsegg', image: '/Images/car.svg', desc: 'Sport' },
              { title: 'Nissan GT - R', image: '/Images/car2.svg', desc: 'sport' },
              { title: 'Rolls-Royce', image: '/Images/car3.svg', desc: 'Sport' },
              { title: 'All New Rush', image: '/Images/car4.svg', desc: 'Suv' },
              { title: 'CR - V', image: '/Images/car5.svg', desc: 'Suv' },
              { title: 'ALLNEW TERIOS', image: '/Images/car6.svg', desc: 'SUV' },
              { title: 'MGZX Exclusive', image: '/Images/car8.svg', desc: 'Hatchback' },
              { title: 'NEW MGZS', image: '/Images/car9.svg', desc: 'SUV' },
              { title: 'MGZX Exclusive', image: '/Images/car10.svg', desc: 'Hatchback' },
          ...
          {showMore && (
            <div className="sec grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
              {[
                { title: 'MG ZX Exclusive', image: '/Images/car10.svg', desc: 'SUV' },
                { title: 'NEW MG ZS', image: '/Images/car11.svg', desc: 'Sedan' },
                { title: 'New MG ZX Excite', image: '/Images/car.svg', desc: 'Sport' },
              ].map((car, index) => (
                <Card key={index} className="w-full max-w-[304px] mx-auto h-auto flex flex-col justify-between">
                  <CardHeader>
                    <CardTitle className="w-full flex items-center justify-between">
